...
class MOT17Sequence(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample = {}
        sample['img'] = img
        #improving performance as converting one element at a time can be slow
...
class MOT20Sequence(MOT17Sequence):
    ...
    def __init__(self, seq_name=None, dets='', vis_threshold=0.0,
        ...
        self._mot_dir = osp.join(DATA_PATH, 'MOT20')
        self._mot17_label_dir = osp.join(DATA_PATH, 'MOT20')
        ...
        seq_name = self._seq_name
        train_test_str =  'train' if seq_name in self._train_folders else 'test'
        self.seq_path = osp.join(self._mot_dir, train_test_str, seq_name)
